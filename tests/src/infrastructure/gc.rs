...
#[tokio::test]
async fn gc_reports_and_removes_orphaned_object() {
...
#[tokio::test]
async fn gc_on_empty_workspace_is_noop() {
    let workspace = tempdir().expect("temp workspace should create");
    let paths = WorkspacePaths::new(workspace.path());
    paths.ensure_store_dirs().await.expect("store dirs should create");

    let report = gc_workspace(&paths, false).await.expect("gc should succeed");

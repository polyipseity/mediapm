...
| Custom suffix set via `set_suffix` survives `add_bar` of another bar (sync_slot delegation) | `sync_slot_preserves_custom_suffix_on_attach` | [covered] |
...
| Inline comment at compute_used_tool_ids call site says desired_tools, not workflow steps      | Manual review (Phase 1 comment fix)                                                                                                          | [covered] |
| compute_used_tool_ids doc no longer self-contradictory about pruning                          | Manual review (Phase 2 doc fix)                                                                                                              | [covered] |
| All entries in provisioning list are always 'used' (dead branch removal)                      | `all_entries_are_used` (unit)                                                                                                                | [covered] |
| used_tool_ids = desired_tools keys + transitive deps only                                     | All `compute_used_tool_ids` tests (Phase 1+2+3)                                                                                              | [covered] |
...
| `set_prefix(String)` string API parses into components via `prefix_components_from_str`                   | `recording_handle_records_all_ops`, `prefix_components_from_str_tool_name_only`, `prefix_components_from_str_all_fields`                                                                                                                       | [covered] |
| `set_suffix(String)` wraps `SuffixComponents`                                                             | (compilation — trivial delegation)                                                                                                                                                                                                             | [covered] |
...
| Production callers in `provision.rs` use `set_prefix_components` / `set_suffix`                           | (compilation + existing sync integration tests)                                                                                                                                                                                                | [covered] |

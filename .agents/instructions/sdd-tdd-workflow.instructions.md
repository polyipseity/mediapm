...
| Binary entry produces both `_DIR` and binary env var per content-map key               | `write_runtime_env_binary_produces_dir_and_binary` (unit)                                                      | [missing] |
| Dir-only entry produces only `_DIR` env var                                             | `write_runtime_env_dir_produces_dir_only` (unit)                                                               | [missing] |
| No duplicate `_DIR` entries when processing multiple keys per OS                        | `write_runtime_env_mixed_os_produces_no_duplicate_dirs` (unit)                                                 | [missing] |

...
| `prefix_overhead` returns 4 for failed/abandoned, 0 otherwise                                             | `prefix_overhead_failed`, `prefix_overhead_abandoned`, `prefix_overhead_other`                                                                                                                                                                 | [covered] |
...
| `PrefixComponents` struct (tool_name, version, phase, count)                                              | `render_prefix_components_*`                                                                                                                                                                                                                   | [covered] |
...
| `semantic_truncate_prefix` takes `&PrefixComponents` directly (no string re-parsing)                      | `semantic_truncate_prefix_already_fits`, `semantic_truncate_prefix_remove_version`, `semantic_truncate_prefix_remove_version_full`, `semantic_truncate_prefix_remove_version_and_count`, `semantic_truncate_prefix_remove_version_count_phase` | [covered] |
| `semantic_truncate_prefix`: empty, already-fits, hard-truncate, failed-overhead edge cases                | `semantic_truncate_prefix_empty`, `semantic_truncate_prefix_hard_truncate`, `semantic_truncate_prefix_failed_overhead`                                                                                                                         | [covered] |
...
| `semantic_truncate_suffix`: empty, already-fits, no-separator, hard-truncate edge cases                   | `semantic_truncate_suffix_empty_both`, `semantic_truncate_suffix_hard_truncate`, `semantic_truncate_suffix_no_custom`, `semantic_truncate_suffix_custom_only`                                                                                  | [covered] |
...
| Prefix `marker` component (left-msg `[F]`/`[A]` folded into `PrefixComponents`, removed atomically before tool_name shrink) | (TBD — Phase 2: `_marker_removed_before_tool_name`, `_marker_preserved_when_fits`, `_marker_failed_effective_width`)                                                                                                                            | [missing] |
| Normative removal orders: prefix version(progressive) -> count/total -> phase -> marker -> tool_name (progressive) -> fallback; suffix custom(progressive) -> eta -> rate -> elapsed -> count/total -> fallback | (TBD — Phase 1/2: `_version_shrinks_first_long_version`, `_count_total_removed_atomically`, `_phase_removed_atomically`, `_removes_eta`, `_removes_rate`, `_removes_elapsed`, `_removes_count_total`, `_count_total_atomic_no_partial`)        | [missing] |
| count/total stored separately, rendered and trimmed together as one atomic unit                          | (TBD — Phase 1/2: `_count_total_removed_atomically`, `_count_total_atomic_no_partial`, `_fits_count_total`, `_removes_count_total`)                                                                                                             | [missing] |
| `render_suffix_components` replaces `build_right_msg` (eta-only-when-rate guard preserved)               | (TBD — Phase 1: `render_suffix_components_with_rate`, `render_suffix_components_with_rate_and_eta`, `render_suffix_components_eta_suppressed_without_rate`, `render_suffix_components_custom_appended`)                                          | [missing] |
| `render_prefix_components(parts, status)` replaces `build_prefix`; `prefix_overhead` deleted             | (TBD — Phase 2: `render_prefix_components_marker_failed`, `render_prefix_components_marker_abandoned`, `render_prefix_components_normal_states_no_marker`, `render_prefix_components_always_starts_with_reset`)                                  | [missing] |
| Normative truncation spec documented verbatim in code doc comments (both removal orders, storage rule, marker render rule) | (TBD — Phase 5 spec-in-implementation audit)                                                                                                                                                                                                    | [missing] |

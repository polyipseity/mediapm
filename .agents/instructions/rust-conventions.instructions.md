...
Both demos detect CI in their embedded `main_is_exercised` tests, never inside `main()` (see `example-execution-policy.instructions.md`). Deterministic example tests (offline demo) always run their full-sync path — no reduced mode, no CI detection. Nondeterministic example tests (online demo) skip in CI with a documented message and run their full-sync path outside CI. `main()` is deterministic given environment inputs and never probes CI variables itself. Manual reduced-mode runs use the documented env override (`MEDIAPM_DEMO_RUN_SYNC=false` or `MEDIAPM_DEMO_ONLINE_RUN_SYNC=false`); manual full runs stay `cargo run --example <demo>`. When reduced mode is active, neither ffmpeg, yt-dlp, rsgain, nor media-tagger are required — only the Rust toolchain and workspace dependencies are needed. Full-sync demo tests that are temporarily blocked by unimplemented upstream features (Stream A stubs) are gated with `#[ignore = "...; see TODO.md — remove only on explicit user request"]` — see `TODO.md`.

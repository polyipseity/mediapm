...
- Fetch sources are downloaded concurrently via `buffer_unordered(default_source_fetch_concurrency().min(n))` where `n` is the number of Fetch-type sources. The concurrency is auto-scaled from `available_parallelism()` with a `MEDIAPM_SOURCE_FETCH_CONCURRENCY` env-var override. Each source runs as an independent future with its own cache lookup, HTTP download, and budget advancement. Results are sorted by source index after collection to preserve determinism. GenerateLauncher sources are handled inline (not parallelized). The `items` counter reported to the progress callback is a shared monotone count of *completed* sources (incremented once per finished source), never a reporting future's own index — out-of-order completion would otherwise make the `{completed}/{total}` prefix oscillate.

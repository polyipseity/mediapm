...
- The online demo YouTube-download regression (`online_sync_post_sync_dump`) is gated separately by the **3-level mechanism** (`example_isolation::ci_mode_detected()` skip + `MEDIAPM_RUN_ONLINE_SYNC=1` opt-in), NOT by `--large` or the `large-tests` feature. The two mechanisms are orthogonal: enabling the feature does not run the YouTube test.

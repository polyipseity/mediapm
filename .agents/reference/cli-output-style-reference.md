# CLI Output style reference

Human-readable reference for the full CLI output style specification. Agents use the compressed version in `rust-conventions.instructions.md`; this file contains the complete detail including all visual templates, styling tables, and code patterns.
...
| `console` 0.15 | Terminal detection and styling | `Term::stderr().size()` for width detection, `style()` for ANSI coloring |

Do not add `owo-colors`, `colored`, `termion`, or other styling crates. `console::style()` is the single styling entry point.

## Shared types (mediapm-utils::progress, always available)

### Download-progress types (no indicatif dependency)
...
- `TrackedHandle` — wraps `Arc<SharedState>` with optional `ProgressBar`. Clones share state.

### TrackedHandle methods

`advance(delta)`, `set_position(pos)`, `set_message(msg)`, `set_prefix(prefix)`, `set_total(total)`, `total()`, `finish()`, `finish_success(msg)`, `finish_error(msg)`, `finish_and_clear()`, `abandon()`, `snapshot()`, `is_finished()`. `ProgressHandle` is a deprecated alias.
...
Every progress bar created through `TrackedHandle::new()`, `ProgressGroup::with_overall()`, or `ProgressGroup::add_bar()` automatically enables a steady tick at 100 ms intervals, which keeps the spinner animating even during long periods without position updates (e.g., slow downloads). No manual `tick()` calls are needed. Uses braille dots sequence: `⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏`.

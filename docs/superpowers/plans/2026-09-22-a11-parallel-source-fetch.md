...
**Re-measurement dropped.** The A/B gate cannot be evaluated — both runs hit the 600s demo watchdog and the env-var control was a no-op. Parallel fetch is kept on correctness merits: tests pass, code is correct, no regressions. The invalid measurement data is recorded in `docs/superpowers/plans/2026-09-22-a11-measurement-results.md`.

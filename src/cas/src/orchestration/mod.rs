//! Actor-based orchestration for CAS storage workflows.
//!
//! This top-level module coordinates storage, index persistence, and
//! maintenance behavior across dedicated actors.
//!
//! ## Topology
//!
//! A typical local stack is composed as:
//!
//! ```text
//! CasNodeActor
//!   ├─> StorageActor
...
//! ```
//!
//! The node actor provides a distributed-ready command façade (`CasWire*`),
//! while the storage actor owns data-path operations and optionally coordinates
//! optimizer/index side effects.
//!
//! Files are split by concern:
//! - `messages.rs`: actor/wire message contracts,
//! - `state.rs`: actor and runtime state types,
//! - `runtime.rs`: `ractor::Actor` implementations + internal orchestration,
//! - `clients.rs`: typed actor clients,
//! - `spawn.rs`: supervision/spawn helpers.
...
pub use clients::{CasNodeActorClient, IndexActorClient, OptimizerActorClient, StorageActorClient};
pub use messages::{
    CasNodeActorMessage, CasWireCommand, CasWireResponse, IndexActorMessage, OptimizerActorMessage,
    StorageActorArgs, StorageActorMessage,
};
pub use spawn::{
    spawn_cas_node_actor, spawn_cas_node_actor_from_refs, spawn_index_actor, spawn_optimizer_actor,
    spawn_storage_actor, spawn_storage_actor_with_dependencies,
};
...
mod tests {
    ...
    use crate::{
        CasApi, CasWireCommand, CasWireResponse, Constraint, FileSystemCas, Hash,
        spawn_cas_node_actor, spawn_index_actor, spawn_optimizer_actor, spawn_storage_actor,
    ...
    async fn cas_node_actor_delete_preserves_delta_descendants() {
        ...
        let _ = node
            .execute(CasWireCommand::OptimizeOnce)
            .await
            .expect("optimize after set_constraint");

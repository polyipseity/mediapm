...
async fn distinct_content_distinct_hashes() {
    let cas = new_in_memory_cas();
    let h1 = put_static(&cas, b"alpha").await;
    let h2 = put_static(&cas, b"beta").await;
    assert_ne!(h1, h2);
}
...
async fn bulk_put_and_get() {
    ...
    for (hash, expected) in hashes.into_iter().zip(items) {
        let retrieved = cas.get(hash).await.unwrap();
...
async fn empty_sentinel_is_always_present() {
    ...
    assert_eq!(empty, Hash::from_content(b""));
    ...
    let base = Hash::from_content(b"some-base");
    ...
    cas.patch_constraint(
        empty,
        ConstraintPatch {
            add_bases: BTreeSet::from([Hash::from_content(b"another")]),

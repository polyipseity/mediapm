...
from `WalPosition::ZERO`), rebuilding Metadata + constraint map. The WAL contains the complete
...
via blanket impls in `store.rs` for any `T: Deref<Target = CasStore<J, M, B>>`.

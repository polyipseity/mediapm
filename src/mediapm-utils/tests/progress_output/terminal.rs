...
fn overflow_h3_5children() {
    let (_mp, term, _o, _kids) = overflow_scaffold(3, 5);
...
fn overflow_h3_5children_clear_last() {
    let (_mp, term, o, kids) = overflow_scaffold(3, 5);
    // At H=3: first 3 children visible (child0, child1, child2).
    assert_eq!(
        term.contents(),
        concat!(
...
fn overflow_h3_10children() {
    let (_mp, term, _o, _kids) = overflow_scaffold(3, 10);
    assert_eq!(
        term.contents(),
        concat!(
            "  child0 [00:00:00] ░░░░░░░░░░░░░░░ 0/3\n",
...
fn overflow_h4_6children_finish_clear_all() {
    let (_mp, term, o, kids) = overflow_scaffold(4, 6);
...
fn worker_surge_with_overflow() {
    let (mp, term, o, mut kids) = overflow_scaffold(3, 5);
...
fn terminal_exact_h2_no_overall_two_children() {
    ...
    group.tick();
    assert_eq!(term.contents(), "⠹                    alpha   0/3 0s 0/d\n",);
}

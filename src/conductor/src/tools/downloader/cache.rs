//! User-level tool download cache engine.
//!
//! This module provides two layers:
//! - default user-cache root helpers for each consuming crate,
//! - one CAS-backed logical-key cache engine reusable across crates.
//!
//! The cache layout is consistent for all callers:
//! - `<cache-root>/store/` for CAS payload objects,
//! - `<cache-root>/*.jsonc` for one or more key-to-hash metadata indexes.
//!
//! The default `<cache-root>` differs by invoking crate:
//! - conductor standalone: `<os-cache-dir>/mediapm-conductor/cache`
//! - mediapm: `<os-cache-dir>/mediapm/cache`
//!
...
///
/// Path policy:
/// - Windows: `%LOCALAPPDATA%/mediapm-conductor/cache`
/// - macOS: `$HOME/Library/Caches/mediapm-conductor/cache`
/// - Linux/Unix: `$XDG_CACHE_HOME/mediapm-conductor/cache`
///   (fallback `$HOME/.cache/mediapm-conductor/cache`)
#[must_use]
pub fn default_user_download_cache_root() -> Option<PathBuf> {
    dirs::cache_dir().map(|root| root.join("mediapm-conductor").join("cache"))
}

/// Returns the default user-scoped tool download cache root for `mediapm`
/// invocations.
///
/// Path policy:
/// - Windows: `%LOCALAPPDATA%/mediapm/cache`
/// - macOS: `$HOME/Library/Caches/mediapm/cache`
/// - Linux/Unix: `$XDG_CACHE_HOME/mediapm/cache`
///   (fallback `$HOME/.cache/mediapm/cache`)
#[must_use]
...
mod tests {
    ...
    fn default_cache_roots_use_flat_cache_layout() {
        ...
        if let (Some(conductor_root), Some(mediapm_root)) = (conductor_root, mediapm_root) {
            ...
            // Base dirs differ: mediapm-conductor vs mediapm

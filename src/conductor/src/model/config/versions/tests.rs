...
use super::{
    USER_NICKEL_VERSION, decode_machine_document, decode_user_document, encode_machine_document,
...
fn atomic_migration_pairs_are_bidirectional_and_adjacent() {
    ...
    for pair in &metadata.atomic_migration_pairs {
        ...
        assert_eq!(delta, 1, "atomic migration edge {} -> {} must be adjacent", pair.from, pair.to);
    }
...
fn atomic_migrations_round_trip_both_directions() {
    ...
    for pair in &metadata.atomic_migration_pairs {
        ...
        let backward: v_latest::NickelEnvelopeLatest =
            migrate_document_source_atomic(forward_source, pair.to, pair.from, "atomic-backward")
...
"#;
    ...
    let err = decode_user_document(source.as_bytes()).expect_err("invalid max_retries should fail");
...
";
    ...
    let err = decode_user_document(source.as_bytes())
        .expect_err("user document should reject impure timestamp subsec_nanos >= 1_000_000_000");
...
";
    ...
    assert!(
        err.to_string().contains("may only define version, impure_timestamps, and state_pointer")
    );
}

...
/// Timezone-independent conductor instance-key derivation timestamp, distinct
/// from the mediapm-level synthesis marker.
///
/// Conductor uses this timestamp during execution planning to derive
/// per-instance cache keys (see `derive_instance_key`). The value represents
/// when the step was planned for execution by conductor, not when the step
/// was generated by mediapm workflow synthesis.
///
/// In contrast, the mediapm-level `MediaPmImpureTimestamp` (in the `mediapm`
/// crate) is a synthesis-time marker that lives in the local `state.ncl` and
/// has no role in conductor cache-key derivation.
///
/// This type uses Unix epoch UTC split into integral components to preserve
/// nanosecond precision without relying on large floating-point numbers in
/// Nickel serialization.

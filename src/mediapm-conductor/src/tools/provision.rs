...
//! conductor resolves the correct executable at runtime.
...
#[cfg(feature = "tool-presets")]
use crate::tools::catalog::{ARCHIVE_BINARY, ToolCatalogEntry};
#[cfg(feature = "tool-presets")]
use crate::tools::model::DownloadProgressCallback;
#[cfg(feature = "tool-presets")]
use crate::tools::resolver::{extract_archive, fetch_bytes_from_candidates, resolve_download_plan};
...
///
/// `progress_cb` is an optional callback invoked after each per-OS download
/// chunk with the current progress snapshot.
///
/// Returns `Ok(None)` when the tool has no catalog entry or is an internal
/// launcher.
#[cfg(feature = "tool-presets")]
pub async fn fetch_and_import_tool_payload(
    cas: &impl CasApi,
    entry: &ToolCatalogEntry,
    cache: &UserLevelCache,
    progress_cb: Option<DownloadProgressCallback>,
) -> Result<Option<ProvisionResult>, ConductorError> {
    let plan = resolve_download_plan(entry, cache).await?;
    ...
    if plan.per_os_actions.is_empty() {
        tracing::warn!("tool {}: no download actions defined, skipping provisioning", entry.id);
        return Ok(None);
    }

    let temp_root = tempfile::tempdir().map_err(|source| {
        ConductorError::io("creating temp directory for tool extraction", PathBuf::new(), source)
    })?;
    ...
    for (os, action) in &plan.per_os_actions {
        ...
        let bytes = if let Some(cached) = cache.lookup_bytes(&cache_key).await {
            ...
            let downloaded = fetch_bytes_from_candidates(&action.urls, cb).await.map_err(|e| {
                ConductorError::Workflow(format!(
                    "tool {}: download failed for {os_label}: {e}",
                    entry.id,
                ))
            })?;
    ...
    Ok(Some(ProvisionResult { content_map, command_selector }))
}
...
/// [`ARCHIVE_BINARY`] the binary is imported as a file-level entry
/// (`./{os_label}/{tool_id}`).
///
/// # Returns
///
...
async fn process_downloaded_archive(
    ...
    os_dir: &Path,
    cas: &impl CasApi,
) -> Result<(BTreeMap<String, String>, String), ConductorError> {
    ...
    if archive_format == ARCHIVE_BINARY {
        ...
        if old_path.exists() {
            std::fs::rename(&old_path, &tool_path).map_err(|source| {
                ConductorError::io(
                    &format!(
...
/// # Errors
///
/// Returns [`ConductorError`] when directory reading or ZIP writing fails.
#[cfg(feature = "tool-presets")]
fn pack_directory_to_uncompressed_zip_bytes(dir: &Path) -> Result<Vec<u8>, ConductorError> {
    let mut buf = Vec::new();
    {
...
mod tests {
    ...
    use super::*;
    use crate::tools::catalog::ARCHIVE_TAR_GZ;
    use crate::tools::catalog::ARCHIVE_TAR_XZ;
    use crate::tools::catalog::ARCHIVE_ZIP;
    ...
    async fn process_zip_archive_linux_label() {
        ...
        let (cmap, exec) =
            process_downloaded_archive(&zip, ARCHIVE_ZIP, "linux", "sd", os_dir.path(), &cas)
                .await
                .unwrap();
    ...
    async fn process_tar_gz_archive_macos_label() {
        ...
        let os_dir = tempfile::tempdir().unwrap();
        let (cmap, exec) =
            process_downloaded_archive(&tgz, ARCHIVE_TAR_GZ, "macos", "sd", os_dir.path(), &cas)
                .await
                .unwrap();
        assert_eq!(cmap.len(), 1);
    ...
    async fn process_tar_xz_archive_windows_label() {
        ...
        let os_dir = tempfile::tempdir().unwrap();
        let (cmap, exec) =
            process_downloaded_archive(&txz, ARCHIVE_TAR_XZ, "windows", "sd", os_dir.path(), &cas)
                .await
                .unwrap();
        assert_eq!(cmap.len(), 1);
    ...
    async fn process_all_three_os_labels_independently() {
        ...
        for os_label in ["linux", "macos", "windows"] {
            let os_dir = tempfile::tempdir().unwrap();
            let (cmap, exec) =
                process_downloaded_archive(&zip, ARCHIVE_ZIP, os_label, "sd", os_dir.path(), &cas)
                    .await
                    .unwrap();
    ...
    async fn process_multi_file_archive_produces_one_entry() {
        ...
        let (cmap, exec) =
            process_downloaded_archive(&zip, ARCHIVE_ZIP, "linux", "sd", os_dir.path(), &cas)
                .await
                .unwrap();
        assert_eq!(cmap.len(), 1, "multi-file archive should produce one dir entry");
        assert!(cmap.contains_key("linux/"), "key should be 'linux/'");
        assert_eq!(exec, "bin/sd");
    }
}

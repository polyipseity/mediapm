...
use self::yt_dlp_inputs::{
    decode_yt_dlp_output_variant_config, resolve_step_output_binding, step_output_policy_overrides,
};
...
pub(super) fn synthesize_media_steps(
    ...
    for (step_index, step) in source.steps.iter().enumerate() {
        ...
        if matches!(step.tool, MediaStepTool::MediaTagger) {
            self::media_tagger::synthesize_media_tagger_step_pair(
                workflow,
                media_id,
                step_index,
                &resolved_step,
            ...
            self::rsgain::synthesize_rsgain_step_chain(
                workflow,
                media_id,
                step_index,
                &resolved_step,
            ...
            self::yt_dlp::synthesize_yt_dlp_step(
                workflow,
                media_id,
                source,
                step_index,
            ...
            self::ffmpeg::synthesize_ffmpeg_step(
                workflow,
                media_id,
                step_index,
                &resolved_step,

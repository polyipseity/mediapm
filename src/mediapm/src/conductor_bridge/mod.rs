...
mod tests {
    ...
    use mediapm_conductor::{
        InputBinding, MachineNickelDocument, OutputCaptureSpec, RuntimeStorageConfig,
        ToolConfigSpec, ToolKindSpec, ToolSpec, default_runtime_inherited_env_vars_for_host,
    ...
    use crate::conductor_bridge::runtime_storage::normalize_runtime_storage_defaults;
        ...
        let existing = ToolConfigSpec {
            ...
            input_defaults: BTreeMap::from([
                ...
                (
                    ...
                    InputBinding::StringList(vec!["-hide_banner".to_string()]),
                ),
            ]),
            env_vars: BTreeMap::new(),
            content_map: None,
    ...
    fn build_tool_command_includes_container_conditional_faststart_and_cues_to_front() {
        ...
        let has_ismv_alias = command.iter().any(|t| t.contains("inputs.container == \"ismv\""));
        assert!(
            has_mp4_flag_token,
        ...
        let has_mkv_value_token = command
            .iter()
            .any(|t| t.contains("inputs.container == \"mkv\"") && t.contains(" 1 | ''}"));
    ...
    fn build_tool_command_sets_media_tagger_flags() {
        ...
        assert!(
            command.iter().any(|token| {
                token
                    == "${*inputs.strict_identification == \"true\" ? --strict-identification | ''}"
            }),
        ...
        assert!(
            command
                .iter()
                .any(|token| token
                    == "${*inputs.cover_art_slot_count ? --cover-art-slot-count | ''}"),
    ...
    fn build_tool_command_includes_sleep_subtitles_tokens() {
        ...
        assert!(
            command
                .iter()
                .any(|token| token == "${*inputs.sleep_subtitles ? --sleep-subtitles | ''}"),
    ...
    fn build_tool_command_always_passes_sd_file_operand() {
        ...
        assert!(
            command
                .iter()
                .any(|token| token == "${inputs.input_content:file(inputs/input.ffmeta)}"),
    ...
    fn build_tool_spec_marks_network_tools_as_impure() {
        ...
        let ffmpeg =
            build_tool_spec(&paths, "ffmpeg", &ffmpeg_payload, default_ffmpeg_slot_limits());
    ...
    fn build_tool_env_exposes_media_tagger_launcher_binding() {
        ...
        let launcher_binary =
            launcher_dir.join(if cfg!(windows) { "mediapm.exe" } else { "mediapm" });

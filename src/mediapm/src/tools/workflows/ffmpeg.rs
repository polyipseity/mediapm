...
use mediapm_conductor::{WorkflowSpec, WorkflowStepSpec};
...
use super::{
    ...
    INPUT_LEADING_ARGS, INPUT_TRAILING_ARGS, VariantProducer, extract_step_list_args,
    ffmpeg_input_content_name, ffmpeg_output_capture_name, ffmpeg_output_path_input_name,
    ffmpeg_output_path_with_extension, normalize_output_extension, resolve_input_variant_producer,
    resolved_ffmpeg_family_output_extension, step_option_input_bindings,
};
...
pub(super) fn synthesize_ffmpeg_step(
    ...
    if let Some(container) = inputs.get_mut("container") {
        *container = canonicalize_ffmpeg_container(container);
    }
    if !inputs.contains_key("movflags") {
        inputs.insert("movflags".to_string(), String::new());
    }
    ...
    for mapping in mappings {
        ...
        inputs.insert(
            ...
            ffmpeg_output_path_with_extension(output_index, resolved_extension.as_deref()),
        );
    ...
    let should_infer_container = match inputs.get("container") {
        Some(container) => container.trim().is_empty(),
        _ => true,
    };
    if should_infer_container && let Some(inferred_container) = inferred_primary_container {
        inputs.insert("container".to_string(), canonicalize_ffmpeg_container(&inferred_container));
    }

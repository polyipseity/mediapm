...
//! - Level 3: set `MEDIAPM_RUN_ONLINE_SYNC=1` to run the full download.
//!
//! Run (Level 3):
//! `MEDIAPM_RUN_ONLINE_SYNC=1 cargo test -p mediapm online_sync_post_sync_dump -- --nocapture --test-threads=1`
...
async fn online_sync_post_sync_dump() {
    ...
    // env var unset). Level 3: explicit MEDIAPM_RUN_ONLINE_SYNC=1 runs
    // the full YouTube download.
    if example_isolation::ci_mode_detected() || !example_isolation::run_online_sync_enabled() {
        eprintln!(
            "[online_sync_post_sync_dump] skipping: set MEDIAPM_RUN_ONLINE_SYNC=1 \
    ...
    assert!(
        fixture.is_file(),
        "generate fixture first: MEDIAPM_RUN_ONLINE_SYNC=false cargo run --example mediapm_demo_online"
    );
